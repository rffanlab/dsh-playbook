      ...
      function PlaybookPanel({ clientCtx }) {
        ...
        const review = async operation => {
          ...
          try {
            ...
            const reviewResult = await runCommand(clientCtx, `/playbook review ${operation} ${target}${feedback}`, expectedSessionId)
            if (sessionIdNow() !== expectedSessionId) return
            await refresh(true)
            setMessage({ error: false, text: operation === 'accept' ? '已记录用户验收通过。' : reviewResult.split('\n')[0] })
          } catch (error) {
        ...
        return h('div', { className: 'dpb' }, [
          ...
          h('main', { className: 'dpb-main', key: 'main' }, [
            ...
            status?.revisionDispatch ? h('div', {className:'dpb-note',key:'dispatch'}, `返修执行消息：${status.revisionDispatch.state} · ${status.revisionDispatch.messageId}（排队/已接收不等于任务完成）`) : null,
