          ...
          h('main', { className: 'dpb-main', key: 'main' }, [
            h('div', { className: 'dpb-head', key: 'head' }, [
              ...
              h('div', { className: 'dpb-actions' }, [
                selectedPb && !status?.active ? h('button', { className: 'dpb-btn', 'data-p': true, disabled: busy, onClick: () => start(selectedPb.id) }, `启动 ${selectedPb.id}`) : null,
                status?.active ? h('button', { className: 'dpb-btn', 'data-d': true, disabled: busy, onClick: cancel }, '取消运行') : null,
              ]),
            ]),
