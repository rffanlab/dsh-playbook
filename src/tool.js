...
export function playbookDefinition(engine, reloadCatalog, router, ready = async () => {}, runChecks, exportReport, projects) {
  ...
  return {
    name: 'playbook',
    description: 'Select a method for the requested deliverable, not nouns in an API specification or project name. Configuring media tools, writing code/documents, making an image/audio and reviewing an existing video are NOT video production. Read referenced task documents before selecting a SOP. Use intake_status to see real read call IDs, then intake to identify the project, task requirements and sources. Project SOPs use sop_list/inspect/validate/save; new versions are immutable trials, protected changes become non-executable drafts until human approval. Never rename/rewrite SOPs to evade a failing gate. Topics/platforms are task inputs, not separate workflows. Choose and execute a task SOP only when one clearly applies. Playbook is optional: when recommend/route reports passthrough, continue the user task normally and do not force task-intake or another vaguely related SOP. recommend is read-only and inspect shows the complete SOP. Multiple complete matches: select a suitable playbook_id with note explaining the fit, or ask only for missing task requirements. Never ask the user to memorize SOP names. Active runs are never replaced by route. Do actual work before submitting evidence. Use check for a read-only preflight and repair format errors, not fabricated values. A failed SOP still governs work: use bounded repair with a target stage and diagnosis; never work outside it. The exact {item:[...]} array mistake is corrected and recorded losslessly. Video submit runs independent Host media checks; self-reported release_ready cannot replace them. Missing inputs/tools: block with a reason. A blocked or terminal run is not completion. report exposes an audit trace. cancel/resume are human commands, not model escape hatches.',
    parameters: {
      ...
      base_id: { type: 'string', description: 'Installed base contract from inspect. Only a produced-video deliverable needs a video base; subject matter and project identity never impose video production.' },
