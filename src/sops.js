...
export const SOP_PLAYBOOKS = [
  ...
  book('task-intake', '结构化接单（可选） / Structured intake (opt-in)', '仅在用户明确需要规划、拆解或结构化接单时使用；没有专用 SOP 的普通任务应由 Harness 正常执行。', [], [
    stage('clarify', '明确交付目标 / Clarify the deliverable', ['This workflow was explicitly selected for structured intake/planning. Ask only for missing goal/input/constraint facts that materially change execution.', 'Do not use this workflow merely because no dedicated SOP exists. Ordinary clear tasks should execute normally without Playbook.'], [text('goal'), items('inputs'), items('acceptance_criteria')]),
